...
public class Enemy extends AnimatedSprite {
	...
	// animation object to simulate moving ghost.
	private Animation moving;
	private Animation death;


	public Enemy ( String id, Layer parent, DifficultyManager running_dm ) {
		super(id, "coolghost.png", 32);
		// increase size of ghost sprites
		this.setScaleX(1.5);
		this.setScaleY(1.5);

		// image array for animation sprites
		BufferedImage[] d = {this.getSprite(0,0), this.getSprite(1,0), 
					this.getSprite(2,0), this.getSprite(3,0),
					this.getSprite(4,0), this.getSprite(5,0),
					this.getSprite(6,0), this.getSprite(7,0),
					this.getSprite(8,0)};

		this.death = new Animation(d, 6);
		
		BufferedImage[] m = {this.getSprite(0,2), this.getSprite(1,2), 
					this.getSprite(2,2), this.getSprite(3,2)};
		
		this.moving = new Animation(m, 6);

		this.setAnimation(this.moving);
	...
	public void handleCollision() {
		...
		int d_x = Parameters.TWEEN_LOCATION_BASE + 
				point_gen.nextInt(Parameters.TWEEN_LOCATION_RANGE) + player_x;
		int d_y = - Parameters.TWEEN_LOCATION_BASE - 
				point_gen.nextInt(Parameters.TWEEN_LOCATION_RANGE) + player_y;

		this.dispatchEvent(new PlayerDamageEvent ( this ) ); // dispatches an event that indicates that health should go down

		enemy_tween_out.animate(TweenableParameter.ALPHA, Parameters.ENEMY_ALPHA, 0, 
					Parameters.FADE_OUT_TIME);
		...
		enemy_tween_out.animate(TweenableParameter.SCALE_X, 1.0,  Parameters.TWEEN_FINAL_SCALE,  
					Parameters.FADE_OUT_TIME);
		enemy_tween_out.animate(TweenableParameter.SCALE_Y, 1.0,  Parameters.TWEEN_FINAL_SCALE,  
					Parameters.FADE_OUT_TIME);
	...
	public void handleDeath( ) {
		...
		enemy_tween_out.animate(TweenableParameter.ALPHA,  
					Parameters.ENEMY_ALPHA, 0,  Parameters.FADE_OUT_TIME);
		enemy_tween_out.animate(TweenableParameter.X, cur.x, d_x,  Parameters.FADE_OUT_TIME);
		enemy_tween_out.animate(TweenableParameter.Y, cur.y, d_y,  Parameters.FADE_OUT_TIME);
		enemy_tween_out.animate(TweenableParameter.SCALE_X, 1.0,  Parameters.TWEEN_FINAL_SCALE,  
					Parameters.FADE_OUT_TIME);
